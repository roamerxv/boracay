...
//退出登录
    $('.Sign_out').on('click',function () {
        ...
        window.sessionStorage.removeItem("lyriceID")
        window.sessionStorage.removeItem('song_title')
    ...
    $('.song_name').text(window.sessionStorage["song_title"]);
    ...
    $(document).on("input  propertychange", "input[name=income]",  function () {
        ...
        $('input[name=income]').each(function (i,item) {
            var value = Number($(item).val().replace('%'));
            sum += value;
    ...
    var albumSong = {
        getHolderList: function () {//获取权利人
            ...
            var song = JSON.parse(window.sessionStorage['param'])[index - 1];//获取歌曲列表
            return song.data['equityList'] || [];//获取权利人数组
        },
        init: function () {/*页面初始化*/
            $('#accordion').html('');
        ...
        saveData: function () {//保存数据到单曲中
            ...
            var songs = JSON.parse(window.sessionStorage['param']);
            var index = window.sessionStorage['lyriceID'];//获取歌曲的索引
            songs[index - 1].data['equityList'] = songholder;
            window.sessionStorage['param'] = JSON.stringify(songs);
