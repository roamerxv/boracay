...
function per_readFile(domClick){
  ...
  reader.onload = function(e){
    showimg(this.result)
    if(fileExt==='.jpg' || fileExt==='.jpeg' || fileExt==='.png'){
      $(domClick).parent().parent().find('#register-view').html('<img class="img" data-name='+fileName+' src="'+this.result+'" alt="" width="300" height="300"/><span class="sign" data-role="delete-file" onclick="javascript:per_remove_img(this)">&times;<span>');
    }else{
      $(domClick).parent().parent().find('#register-view').html('<a target="_blank" class="img filed" download="true">'+fileName+'</a><span class="sign" data-role="delete-file" onclick="javascript:per_remove_img(this)">&times;</span>');
    }
