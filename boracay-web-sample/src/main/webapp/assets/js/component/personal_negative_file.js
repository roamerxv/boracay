 ...
 */
function negative_readFile_view(domClick){
    //var _this=this
    ...
    reader.onload = function(e){
        show_negative_view(this.result)
        window.sessionStorage["baseImg"] = this.result
        if(fileExt==='.jpg' || fileExt==='.jpeg' || fileExt==='.png'){
            $(domClick).parent().parent().find('#card_negative_view').html('<img class="img" data-name='+fileName+' src="'+this.result+'" alt="" width="360px" height="230px"/><span class="baseInfo_sign" data-role="delete-file" onclick="javascript:base_remove_img(this)">&times;<span>');
        }else{
            $(domClick).parent().parent().find('#card_negative_view').html('<a target="_blank" class="img filed" download="true">'+fileName+'</a><span class="sign" data-role="delete-file" onclick="javascript:base_remove_img(this)">&times;</span>');
        }
...
function show_negative_view(data){
    $("#show_negative_view").append("<img id = 'showImg' class = 'hide' src='' />");
...
function dom_show(domName,targetName){
    if($(targetName).parent().parent().find('input').attr('id') === domName){
        ...
        h+='<input class="form-control '+domName+'" type="file" id="'+domName+'" name="'+domName+'" accept="image/bmp,image/jpeg,image/png" onchange="negative_readFile_view(this)">';
