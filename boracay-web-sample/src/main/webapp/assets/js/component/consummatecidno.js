 ...
 */
function base_readFile_view(domClick){
    //var _this=this
    ...
    if(fileSize > 5){
        $('.errorSize_no').html("<p>上传文件超过5兆</p>")
        setTimeout(function(){
            $('.errorSize_no p').fadeOut()
        },2000)
        setTimeout(function(){
            $('.errorSize_no').html('')
        },2000)
...
function dom_show_img(domName,targetName){
    if($(targetName).parent().parent().find('input').attr('id') === domName){
        ...
        h+='<input class="form-control '+domName+'" type="file" id="'+domName+'" name="'+domName+'" accept="image/bmp,image/jpeg,image/png" onchange="base_readFile_view(this)">';
