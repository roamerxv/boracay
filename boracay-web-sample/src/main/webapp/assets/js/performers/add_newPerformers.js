...
$().ready(function() {
  ...
  var performers_confirmBtn = $('#performers_add');
  var performers_confirmBtn  = performers_confirmBtn .find('[data-am-modal-confirm]');
  performers_confirmBtn .off('click.confirm.modal.amui').on('click', function() {
    //验证网站和名称是否填写
    /*var netCheck = false
   ...
   HttpUtils.post_performer_data(param,function(data){
      if(data.status = 200){
        $('#performers_type_select2').append('<option value="'+data.data.localMessage+'">'+perName+'</option>')
        $("#performers_type_select2 option:last").prop("selected", 'selected');
        reload_recordCompany();
        $('#performers_add').modal('close')
      }
    })
  })
