...
public class SmsController {
  ...
  @Autowired private SmsVerificationService smsVerificationService;
  ...
  private String generateCode(String businessMethod) throws BoracayException {
    ...
    for (int i = 0; i < length; i++) {
      sb.append(X.charAt(new Random().nextInt(10)));
    }
    return sb.toString();
  }
  ...
  // 生成短信验证码实体
  private SmsVerificationCodeEntity genereateSmsVerificationCodeEntity(
      String method,
      String sessionId,
      String phoneNumber,
      String text,
  ...
  @ResponseBody
  public void delete(
      @PathVariable("method") String method, @PathVariable("user_mobile") String user_mobile)
    ...
    smsVerificationService.save(
        genereateSmsVerificationCodeEntity(method, sessionId, user_mobile, text, code, duration));
