...
public class UploadFileUtil {
  ...
  /**
   * 把 MultipartFile 的文件保存到服务器端。返回结果。 使用 uuid 作为 id 保存的路径 在 config文件中设置
   *
   * @author 徐泽宇
   ...
   * @throws
   */
  public FileUploadResult saveFile(MultipartFile multipartFile)
      throws IOException, NoSuchAlgorithmException, BoracayException {
    return saveFile(multipartFile, false);
  }

  /**
   * 把 MultipartFile 的文件保存到服务器端。返回结果。 如果 digestIt 是 false ，使用 uuid 作为 id，否则用摘要做 id 保存的路径 在
   * config文件中设置
   *
   * @author 徐泽宇
   ...
   * @throws
   */
  public FileUploadResult saveFile(MultipartFile multipartFile, boolean digestIt)
      throws IOException, NoSuchAlgorithmException, BoracayException {
  ...
  /**
   * 把 MultipartFile 的文件保存到服务器端。返回结果。 使用 uuid 作为 id 保存的路径 通过 savePath 指定
   *
   * @author 徐泽宇
   * @date 2018/5/12 下午7:43
   ...
   */
  public FileUploadResult saveFile(MultipartFile multipartFile, String savePath)
      throws IOException, NoSuchAlgorithmException, BoracayException {
    return saveFile(multipartFile, false, savePath);
  }

  /**
   * 把 MultipartFile 的文件保存到服务器端。返回结果。 如果 digestIt 是 false ，使用 uuid 作为 id，否则用摘要做 id 保存的路径 通过 savePath
   * 指定
   *
   * @author 徐泽宇
   ...
   * @throws
   */
  public FileUploadResult saveFile(MultipartFile multipartFile, boolean digestIt, String savePath)
      throws IOException, NoSuchAlgorithmException, BoracayException {
  ...
  /**
   * 把 MultipartFile 数组中的文件保存到服务器端。返回结果。 使用 uuid 作为 id 保存的路径 在 config文件中设置
   *
   * @author 徐泽宇
   ...
   * @throws
   */
  public ArrayList<FileUploadResult> saveFile(MultipartFile[] multipartFileArrayList)
      throws IOException, NoSuchAlgorithmException, BoracayException {
    return this.saveFile(multipartFileArrayList, false);
  }

  /**
   * 把 MultipartFile 数组中的文件保存到服务器端。返回结果。 保存的路径 在 config文件中设置 是否要通过 摘要作为 id。通过 digestIt 来指定
   *
   ...
   * @throws NoSuchAlgorithmException
   */
  public ArrayList<FileUploadResult> saveFile(
      MultipartFile[] multipartFileArrayList, boolean digestIt)
      throws IOException, NoSuchAlgorithmException, BoracayException {
    String savePath = ConfigHelper.getConfig().getString("System.UploadFile.saveFilePath");
    if (StringUtils.isEmpty(savePath)) {
      throw new BoracayException("system.UploadFile.saveFilePath not setted in config file！");
  ...
  /**
   * 把 MultipartFile 数组中的文件保存到服务器端。返回结果。 保存的路径 通过 savePath 指定 通过 随机生成的 UUID 来作为 目录名
   *
   * @param multipartFileArrayList 需要保存的上传后的文件
   ...
   */
  public ArrayList<FileUploadResult> saveFile(
      MultipartFile[] multipartFileArrayList, String savePath)
      throws IOException, NoSuchAlgorithmException, BoracayException {
   ...
   */
  public ArrayList<FileUploadResult> saveFile(
      MultipartFile[] multipartFileArrayList, boolean digestIt, String savePath)
      throws IOException, NoSuchAlgorithmException, UtilException {
