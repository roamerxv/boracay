...
public class SMSValidateCodeAspect {
  ...
  private static String SMS_VCODE_AND_OPIDS_SIZE_NOT_MATCH =
      "exception.sms.validate.vcode.opid_size_no_match";

  @Autowired private HttpServletRequest request;

  @Autowired private SmsVerificationService smsService;
  ...
  private void smsValidateCodeCheck(JoinPoint joinPoint) throws BoracayException {
    ...
    String[] opIDs = methodAnnotation.opId();
    if (opIDs.length <= 0) {
    } else {
      ...
      log.debug(
          "类[{}]中的[{}]方法使用了@SMSValidateMethodAnnotation注解，并且设置了 opID 是 {}。需要进行短信验证!",
